...
const ExcelReportGenerator = {
  async generateReceiptsReport(receipts: any[], filters: any) {
  ...
  async generateInvoicesReport(invoices: any[], filters: any) {
    const workbook = new ExcelJS.Workbook();
  ...
  async generateFinancialReport(data: any) {
    ...
    ExcelReportGenerator.createRevenueByService(
      revenueSheet,
  ...
  async generateCustomerReport(customers: any[], filters: any) {
    const workbook = new ExcelJS.Workbook();
  ...
  styleHeader(worksheet: ExcelJS.Worksheet) {
  ...
  addSummarySection(worksheet: ExcelJS.Worksheet, data: any[], type: string) {
    ...
    if (type === 'Invoices') {
      ...
      summaryData.push(
        ['Total Amount:', `GMD${totalAmount.toFixed(2)}`],
        ['Paid Amount:', `GMD${paidAmount.toFixed(2)}`],
        ['Unpaid Amount:', `GMD${unpaidAmount.toFixed(2)}`],
      );
    }
  ...
  addFiltersInfo(worksheet: ExcelJS.Worksheet, filters: any) {
    const lastRow = worksheet.rowCount + 2;
  ...
  createFinancialSummary(worksheet: ExcelJS.Worksheet, summary: any) {
    worksheet.columns = [
  ...
  createPaymentsDetail(worksheet: ExcelJS.Worksheet, payments: any[]) {
    worksheet.columns = [
  ...
  createRevenueByService(worksheet: ExcelJS.Worksheet, revenueData: any[]) {
    worksheet.columns = [
  ...
  async saveExcelToFile(excelBuffer: any, filename: string): Promise<string> {
