...
const PDFGenerator = {
  ...
  async generateReceiptPDF(receiptData: any): Promise<Buffer> {
    try {
      console.log(
        ...
        receiptData ? 'data present' : 'no data',
      );
  ...
  async generateReceiptsReportPDF(
    receiptsData: any[],
    filters: any,
  ): Promise<Buffer> {
    try {
      console.log(
        '🔍 PDF Generator: generateReceiptsReportPDF called with',
        receiptsData.length,
        'receipts',
      );
  ...
  generateReceiptHTML(receiptData: any): string {
    console.log(
      'PDF Generator received receiptData:',
  ...
  async generateInvoicesReportPDF(
    invoices: any[],
    filters: any,
  ): Promise<Buffer> {
    const browser = await this.initBrowser();
  ...
  generateInvoicesReportHTML(invoices: any[], filters: any): string {
  ...
  generateReceiptsReportHTML(receiptsData: any[], filters: any): string {
    console.log(
      '🔍 PDF Generator: generateReceiptsReportHTML called with',
      receiptsData.length,
      'receipts',
    );
    ...
    const totalAmount = receiptsData.reduce((sum, receipt) => {
      ...
      return (
        sum + (typeof amount === 'string' ? parseFloat(amount) : Number(amount))
      );
    }, 0);
          ...
          <tbody>
            ${receiptsData
              ...
              `;
                ...
                console.log(
                  '🔍 PDF Generator: Generated row HTML:',
                  rowHtml.trim(),
                );
                return rowHtml;
              })
  ...
  buildFilterInfo(filters: any): string {
    const parts = [];
    if (filters.startDate && filters.endDate) {
