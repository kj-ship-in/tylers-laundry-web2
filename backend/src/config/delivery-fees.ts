 ...
 */
export const detectZoneFromAddress = (address: string): string | undefined => {
...
export const calculateDeliveryFee = (
  ...
  if (deliveryAddress) {
    ...
    if (detectedZone) {
      const zoneConfig =
        DELIVERY_FEE_CONFIG.ZONES[
          detectedZone as keyof typeof DELIVERY_FEE_CONFIG.ZONES
        ];
      fee = zoneConfig.fee;
    }
  }
  ...
  if (
    serviceType &&
    DELIVERY_FEE_CONFIG.SERVICE_SURCHARGES[
      serviceType as keyof typeof DELIVERY_FEE_CONFIG.SERVICE_SURCHARGES
    ]
  ) {
    const surcharge =
      DELIVERY_FEE_CONFIG.SERVICE_SURCHARGES[
  ...
  if (
    DELIVERY_FEE_CONFIG.WEIGHT_BASED.enabled &&
    weight &&
...
export const applyBulkDiscount = (
  ...
  if (
    DELIVERY_FEE_CONFIG.BULK_DISCOUNT.enabled &&
    subtotal >= DELIVERY_FEE_CONFIG.BULK_DISCOUNT.minimumAmountRequired
  ) {
    const bulkDiscount =
      (subtotal * DELIVERY_FEE_CONFIG.BULK_DISCOUNT.percentageDiscount) / 100;
