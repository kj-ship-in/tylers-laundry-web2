...
const emptyPagination = (page: number) => ({
  currentPage: page,
  totalPages: 0,
  totalUsers: 0,
  hasNext: false,
...
export const getCustomersService = async (options?: {
  ...
  if (!userRole) return { users: [], pagination: emptyPagination(page) };

  const query: any = { roleId: userRole._id };
...
export const getStaffsService = async (options?: {
  ...
  if (!staffRole) return { users: [], pagination: emptyPagination(page) };

  const query: any = { roleId: staffRole._id };
  if (!includeDeleted) query.deletedAt = null;
  if (search) {
    query.$or = [
      { name: { $regex: search, $options: 'i' } },
...
export const getCurrentUserService = async (userId: string): Promise<any> => {
...
export const updateProfilePictureService = async (userId: string, profileUrl: string) => {
