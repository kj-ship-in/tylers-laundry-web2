...
const ReportingService = {
  async generateFinancialReport(filters: {
    ...
    const revenueByServiceWithDetails = revenueByService.map((revenue: any) => {
      const service = services.find(s => (s._id as any).toString() === revenue._id.toString());
      return {
  ...
  async generateCustomerReport(filters: {
    ...
    if (filters.format === 'excel') {
      const excelBuffer = await ExcelReportGenerator.generateCustomerReport(
        customersWithStats,
        filters,
      );
    ...
    const reportData = await Promise.all(
      serviceStats.map(async service => {
        ...
        return {
          ...
          conversionRate:
            bookings.length > 0 ? (completedCount / bookings.length) * 100 : 0,
        };
      }),
    );
  ...
  async getRevenueLineChartData(days: number = 30) {
  ...
  async getBookingStatusBarChartData() {
    ...
    const data = bookingsByStatus.map((item: any) => ({
  ...
  async getServiceRevenueDonutData() {
    ...
    const donutData = serviceRevenue
      .map((item: any) => {
        const service = services.find(s => (s._id as any).toString() === item._id.toString());
        return {
    ...
    return {
      labels: donutData.map((d: any) => d.serviceName),
      datasets: [
        {
          label: 'Revenue by Service (GMD)',
          data: donutData.map((d: any) => d.revenue),
  ...
  async getPaymentMethodDonutData() {
    ...
    const data = paymentMethods.map((item: any, index: number) => ({
  ...
  async getDailyBookingComparisonData(days: number = 30) {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);

    const bookings = await Booking.find({ createdAt: { $gte: startDate } }).select(
      'createdAt status',
    );
  ...
  async getCustomerAcquisitionLineData(months: number = 12) {
    const startDate = new Date();
    ...
    const chartData = Array.from({ length: months }, (_, index) => {
      ...
      const found = customersByMonth.find(
        (d: any) => d._id.year === month.getFullYear() && d._id.month === month.getMonth() + 1,
      );
      return {
  ...
  async getStaffPerformanceMetrics() {
    ...
    const staffMembers = await User.find({ roleId: staffRole?._id, isActive: true }).select(
    ...
    return Promise.all(
      staffMembers.map(async staff => {
        ...
        const [
          ...
          Booking.find({ status: BookingStatus.COMPLETED, createdAt: { $gte: staff.createdAt } }).select(
            'createdAt updatedAt',
          ),
        ]);

        const completionTimes = completionTimeDocs.map(
          b => new Date(b.updatedAt).getTime() - new Date(b.createdAt).getTime(),
        );
        const avgMs =
  ...
  async getStaffWorkloadDistribution() {
    ...
    const workloadData = await Promise.all(
      staffMembers.map(async staff => {
        const [pendingBookings, inProgressBookings, completedToday, completedThisMonth] =
          await Promise.all([
            Booking.countDocuments({ status: BookingStatus.PENDING }),
  ...
  async getStaffEfficiencyReport(startDate?: string, endDate?: string) {
    ...
    const efficiencyData = await Promise.all(
      staffMembers.map(async staff => {
        const [totalBookings, completedBookings, revenueGenerated, avgOrderResult] =
          await Promise.all([
