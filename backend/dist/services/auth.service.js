...
export const registerUserService = async (data) => {
    ...
    await EmailVerification.findOneAndUpdate({ userId: user._id }, { userId: user._id, code, expiresAt, createdAt: new Date() }, { upsert: true, new: true });
    ...
    await Session.findOneAndUpdate({ userId: user._id }, {
        userId: user._id,
        refreshToken,
...
export const registerPrivilegedUserService = async (data, roleName, requestingUserId) => {
    ...
    const requestingUser = await User.findById(requestingUserId).populate('roleId');
    if (!requestingUser)
...
export const registerAdmin = async (data, requestingUserId) => registerPrivilegedUserService(data, 'ADMIN', requestingUserId);
export const registerStaff = async (data, requestingUserId) => registerPrivilegedUserService(data, 'STAFF', requestingUserId);
export const registerUser = async (data) => registerUserService(data);
export const login = async (userId) => {
    const user = await User.findByIdAndUpdate(userId, { lastLogin: new Date() }, { new: true }).populate('roleId');
    if (!user)
...
export const verifyUserEmail = async (userId, code) => {
    ...
    const user = await User.findByIdAndUpdate(userId, { isVerified: true, verifiedAt: new Date() }, { new: true }).populate('roleId');
    if (!user)
        throw new AuthenticationError('User not found');
...
export const requestVerificationCode = async (userId, email) => {
    const code = generateVerificationCode();
    const expiresAt = new Date(Date.now() + CODE_EXPIRATION_MINUTES * 24 * 60 * 60 * 1000);
    await EmailVerification.findOneAndUpdate({ userId }, { userId, code, expiresAt, createdAt: new Date() }, { upsert: true, new: true });
    const expiryTime = getRelativeExpiry(expiresAt);
...
export const requestPasswordReset = async (userId, email) => {
    ...
    await PasswordResetToken.findOneAndUpdate({ userId }, { userId, resetToken, expiresAt }, { upsert: true, new: true });
    const expiryTime = getRelativeExpiry(expiresAt);
...
export const softDeleteUser = async (userId, deletionReason = 'USER_REQUESTED') => {
    ...
    return User.findByIdAndUpdate(userId, { deletedAt: new Date(), email: anonymizedEmail, isActive: false, deletionReason }, { new: true, select: 'id email isActive deletedAt deletionReason' });
};
export const restoreUser = async (userId) => {
    return User.findByIdAndUpdate(userId, { deletedAt: null, isActive: true, deletionReason: null }, { new: true, select: 'id email isActive deletedAt deletionReason' });
};
