...
export const deleteStaffService = async (staffId) => {
    ...
    return User.findByIdAndUpdate(staffId, { deletedAt: new Date(), email: anonymizedEmail, isActive: false, deletionReason: 'ADMIN_DELETED' }, { new: true, select: 'id email isActive deletedAt deletionReason' });
};
export const updateUserStatusService = async (userId, isActive) => {
    const user = await User.findById(userId).populate('roleId');
    if (!user)
        throw new Error('User not found');
    return User.findByIdAndUpdate(userId, { isActive }, { new: true, select: 'id email name isActive' });
};
export const deleteCustomerService = async (customerId) => {
    ...
    return User.findByIdAndUpdate(customerId, { deletedAt: new Date(), email: anonymizedEmail, isActive: false, deletionReason: 'ADMIN_DELETED' }, { new: true, select: 'id email isActive deletedAt deletionReason' });
};
